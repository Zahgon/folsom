...
public class OpenTelemetryMetrics implements Metrics {
    ...
    public OpenTelemetryMetrics(final OpenTelemetry openTelemetry) {
        ...
        this.durations = meter.histogramBuilder("memcache.requests.nanoseconds").setUnit("nanoseconds").build();
        ...
        meter.gaugeBuilder("memcache.outstanding.requests").buildWithCallback(this::outstandingRequestsObservable);
        meter.gaugeBuilder("memcache.global.connections").buildWithCallback(this::globalConnectionsObservable);
    }

    private void outstandingRequestsObservable(final ObservableDoubleMeasurement measurement) {
        measurement.record(outstandingRequests.stream().mapToLong(value -> (long) value.getOutstandingRequests()).sum());
    }
