...
public class KetamaMemcacheClient extends AbstractMultiMemcacheClient {
    ...
    private <T> CompletionStage<T> sendToAll(final AllRequest<T> request) {
        final List<CompletionStage<T>> futures = clients.stream().map(client -> client.send(request.duplicate())).map(request::preMerge).collect(Collectors.toList());
    ...
    private <T> CompletionStage<List<T>> sendSplitRequest(final MultiRequest<T> multiRequest) {
        ...
        return CompletableFuture.allOf(values.toArray(new CompletableFuture<?>[values.size()])).thenApply(new Assembler<>(futures, routing2));
    }
